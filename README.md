...
- `COPY_TRADE_BUY_PRESSURE_SELL_ENABLED=true` enables the buy-pressure exit mode after a confirmed auto copy buy. It watches the copied mint through PumpPortal token trades plus any Helius watched-wallet trade events, sells `COPY_TRADE_BUY_PRESSURE_SELL_PERCENT` after `COPY_TRADE_BUY_PRESSURE_SELL_MIN_BUYS` and optional `COPY_TRADE_BUY_PRESSURE_SELL_MIN_TOTAL_SOL` are met, or sells on `COPY_TRADE_BUY_PRESSURE_SELL_TIMEOUT_MS` as a fallback. Watchers are persisted in `COPY_TRADE_BUY_PRESSURE_SELL_STATE_PATH` and resume timeout fallback after restart; live/dry-run gates, sell slippage/priority settings, and copied-position token balance checks still apply.
