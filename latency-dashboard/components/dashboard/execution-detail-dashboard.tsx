...
function GroupedDiagnostics({ row, timeZone }: { row: DashboardExecution; timeZone: string }) {
  ...
  const executionRows = [
    ...
    { label: "Feed", value: row.inboundSource ? feed.label : "Unknown" },
    { label: "Feed contributors", value: row.inboundContributors.length > 0 ? [...new Set(row.inboundContributors.map((source) => executionFeed(source).label))].join(" → ") : "n/a" },
