...
- `GEYSER_ENABLED=true` starts an optional VA Geyser/Yellowstone gRPC watched-wallet stream at `GEYSER_GRPC_URL`. Keep `GEYSER_SHADOW_ONLY=true` at first; shadow mode writes/logs Geyser candidates but does not trigger copy buys or Telegram alerts. Legacy `YELLOWSTONE_*` env names are still accepted.
- `COPY_TRADE_SIGNAL_PROVIDER=pumpportal` keeps PumpPortal as the copy-buy trigger. Use `parallel` for a canary where PumpPortal and Geyser can race through the same duplicate/idempotency guards, and `geyser` only after the parallel canary is clean.
- To summarize a canary feed window, run `npm run feed-latency -- --log logs/wallet-trades.jsonl`. Add `--app-log <captured service log>` to include Yellowstone parser rejection counts.
- VA RPC/Geyser deploy, canary, and rollback steps live in `docs/va-geyser-runbook.md`.
