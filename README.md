...
Hermes Trader is a Solana-focused Telegram bot for Pump.fun migration and wallet
copy-trade monitoring.
...
- `COPY_TRADE_RUST_EXECUTION_ALERTS_SOURCE=local-jsonl` and `COPY_TRADE_RUST_ASYNC_PLATFORM_FEE_SOURCE=local-jsonl` keep Node-side post-submit readers on `/var/log/jito-copy-executions-vps.jsonl` instead of repeatedly reading `copytrade_local_executions` from Supabase. This does not change the Rust hot path; Rust still owns ShredStream decode, match, build, sign, and send, while Node handles Telegram/cashback work after the local execution row exists. Use Supabase as the dashboard/reporting sink, not as the polling source for these live post-trade workers.
- `COPY_TRADE_RUST_ASYNC_PLATFORM_FEE_ENABLED=true` enables the Node-side post-submit bridge for Rust copy buys. Rust still sends the buy unchanged; Node reads eligible Rust buy rows after submit, creates a pending cashback ledger row from `raw_execution.plannedCopySpendLamports`, sends a separate `SystemProgram.transfer` from the copy wallet to `PLATFORM_FEE_TREASURY`, and makes cashback claimable only after that fee transfer confirms. Keep this default-off until a tiny-wallet rollout verifies `Rust buy row -> pending ledger -> fee transfer signature -> claimable ledger`; enabling requires `COPY_TRADE_RUST_ASYNC_PLATFORM_FEE_CANARY_CHAT_IDS`, `COPY_TRADE_RUST_ASYNC_PLATFORM_FEE_CANARY_WALLETS`, or an explicit `COPY_TRADE_RUST_ASYNC_PLATFORM_FEE_ALLOW_ALL_CHATS=true`.
