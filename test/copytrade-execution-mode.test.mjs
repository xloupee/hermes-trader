...
test("copy trade execution mode blocks live PumpPortal submissions by default", () => {
  ...
  assert.match(formatCopyTradeExecutionStateLog(config), /executionProvider=direct-auto/);
...
test("copy trade execution mode requires enabled and not dry-run for live submissions", () => {
  ...
  assert.match(formatCopyTradeExecutionStateLog(liveConfig), /livePumpPortalSubmissions=blocked/);
});
