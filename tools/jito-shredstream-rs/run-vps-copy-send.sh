...
DEFAULT_JITO_TELEGRAM_SNAPSHOT_PATH="/opt/pumpfun-migration-bot/data/jito-telegram-copy-snapshot.json"
if [[ -n "${JITO_TELEGRAM_SNAPSHOT_PATH:-}" || -f "$DEFAULT_JITO_TELEGRAM_SNAPSHOT_PATH" ]]; then
  export JITO_TELEGRAM_SNAPSHOT_PATH="${JITO_TELEGRAM_SNAPSHOT_PATH:-$DEFAULT_JITO_TELEGRAM_SNAPSHOT_PATH}"
fi
