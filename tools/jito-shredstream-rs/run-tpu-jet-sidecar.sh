...
export JITO_TPU_JET_WS_URL="${JITO_TPU_JET_WS_URL:-${JITO_TPU_JET_GRPC_URL:-${JITO_SHREDER_FASTLANE_GRPC_URL:-${JITO_ERPC_YELLOWSTONE_GRPC_URL:-}}}}"
export JITO_TPU_JET_GRPC_URL="${JITO_TPU_JET_GRPC_URL:-$JITO_TPU_JET_WS_URL}"
export JITO_TPU_JET_GRPC_X_TOKEN="${JITO_TPU_JET_GRPC_X_TOKEN:-${JITO_SHREDER_FASTLANE_GRPC_X_TOKEN:-${JITO_ERPC_YELLOWSTONE_GRPC_X_TOKEN:-}}}"
...
if [[ -z "${JITO_TPU_JET_GRPC_URL:-}" ]]; then
  echo "JITO_TPU_JET_GRPC_URL, JITO_TPU_JET_WS_URL, JITO_SHREDER_FASTLANE_GRPC_URL, or JITO_ERPC_YELLOWSTONE_GRPC_URL must be set in $APP_ENV_FILE or $WORKER_ENV_FILE" >&2
  exit 1
fi
...
if [[ -n "${JITO_SHREDER_FASTLANE_GRPC_URL:-}" && "${JITO_TPU_JET_GRPC_URL:-}" == "${JITO_SHREDER_FASTLANE_GRPC_URL:-}" ]]; then
  echo "  grpc source: shreder fastlane"
elif [[ -n "${JITO_ERPC_YELLOWSTONE_GRPC_URL:-}" && "${JITO_TPU_JET_GRPC_URL:-}" == "${JITO_ERPC_YELLOWSTONE_GRPC_URL:-}" ]]; then
  echo "  grpc source: erpc yellowstone"
else
  echo "  grpc source: direct tpu jet env"
fi
