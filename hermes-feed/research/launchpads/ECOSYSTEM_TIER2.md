...
- The proof calldata independently fixes the standard profile: 7,000 ppm core LP fee, 200 tick spacing, 85% of supply in two 99%/1% liquidity curves, `DopplerHookInitializer` `0x4e34...a544`, and Rehype hook `0x6f02...0f77`. The Rehype schedule is an additional output-token fee, not the V4 LP fee: it decays linearly from 800,000 to 5,000 over ten seconds and divides by 800,000.
- At the normalized creation timestamp the Rehype fee consumes 100% of core output. The first independently quoted nonzero state is therefore `start + 1`, where the rate is 720,500 / 800,000. Under the proof's routing matrix, 5% of the hook fee remains protocol-owned and the other 95% is immediately swapped in the opposite direction; that internal swap must update warm pool state before simulating an immediate exit.
- Immediate WETH/V4 pool; Bankr documents the 0.7% core swap fee as 95% creator/5% Doppler, while the verified Rehype hook above is a separate output fee. Standard launch reserves 15% for two-year vesting with 30-day cliff; 85% seeds liquidity. Partner launches use 100% pool supply. No Bankr-specific graduation is documented. CREATE2 derivation remains unknown.
