  ...
  return {
    async listSubscribers() {
      const { data, error } = await client
        ...
        .select(
          "chat_id,mode,copy_wallet_address,copy_amount_sol,copy_target_wallet_address,verified_at,updated_at,telegram_watched_wallets(address,label,added_at,updated_at)"
        )
